...
export default function DashboardScreen() {
  const { wardrobe, savedOutfits, saveOutfit, incShuffle, shuffleCount, generateSmartOutfit } = useStore();

  const initOutfit = (): Outfit => generateSmartOutfit(wardrobe);
  ...
  const outfitEmpty = !outfit.top && !outfit.bottom && !outfit.shoes && !outfit.accessories;

  React.useEffect(() => {
    if (wardrobe.length === 0) return;
    setOutfit(prev => {
      if (prev.top && prev.bottom && prev.shoes && prev.accessories) return prev;
      return generateSmartOutfit(wardrobe);
    });
  }, [wardrobe, generateSmartOutfit]);
  ...
  const doShuffle = () => {
    ...
    setOutfit(generateSmartOutfit(wardrobe));
    incShuffle();
    setHearted(false);
  };
  ...
  return (
    <ScrollView style={s.screen} showsVerticalScrollIndicator={false}>
      ...
      {outfitEmpty && (
        <View style={s.empty}> 
          <Text style={s.emptyTitle}>Outfit empty</Text>
          <Text style={s.emptySub}>Add wardrobe items to generate your first look.</Text>
        </View>
      )}
      ...
      <View style={s.actionRow}>
        <TouchableOpacity
          ...
          activeOpacity={wardrobe.length > 0 ? 0.85 : 1}
        >
          <Text style={s.shuffleIcon}>⚡</Text>
          <Text style={[s.shuffleText, wardrobe.length === 0 && s.shuffleTextDisabled]}>Shuffle</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[s.heartBtn, hearted && s.heartBtnOn, outfitEmpty && s.heartBtnDisabled]}
          onPress={!outfitEmpty ? doSave : undefined}
          activeOpacity={!outfitEmpty ? 0.8 : 1}
        >
...
const s = StyleSheet.create({
  ...
  heartBtnDisabled: { opacity: 0.5, backgroundColor: C.gray2, borderColor: C.gray3 },
